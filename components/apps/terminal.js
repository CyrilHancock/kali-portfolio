...
export class Terminal extends Component {
    constructor() {
        ...
        this.child_directories = {
            ...
            skills: ["Front-end development", "React.js", "jQuery", "Express.js", "SQL", "Firebase","CSharp","Dotnet","Azure"],
            projects: ["Whatapp Next"],
            ...
            <React.Fragment key={id}>
                <div className="flex w-full h-7">
                    <div className="flex flex-col">
                        <div className='flex'>
                        <span className='text-green-500'>┌──(</span><span className='text-blue-500'>cyril㉿RACIT007</span><span className='text-green-500'>)-[</span><div className=" text-ubt-blue">{this.current_directory}</div><span className='text-green-500'>]</span>
                        </div>
                        <div className='flex'>
                        <span className='text-green-500'>
                        └─
                        </span>
                        <div className="mx-px mr-1 font-medium text-white">
                    ...
                    <div id="cmd" onClick={this.focusCursor} className="relative flex-1 overflow-hidden bg-transperent">
                        <span id={`show-${id}`} className="float-left pb-1 font-normal tracking-wider whitespace-pre opacity-100 "></span>
                        <div id={`cursor-${id}`} className=" float-left mt-1 w-1.5 h-3.5 bg-white"></div>
                        <input id={`terminal-input-${id}`} data-row-id={id} onKeyDown={this.checkKey} onBlur={this.unFocusCursor} className="absolute top-0 left-0 w-full bg-transparent outline-none opacity-0 " spellCheck={false} autoFocus={true} autoComplete="off" type="text" />
                    </div>
                </div>
    ...
    handleCommands = (command, rowId) => {
        ...
        switch (main) {
            ...
            case "pwd":
                ...
                result = str.replace("~", "/home/Cyril")
                break;
            case "code":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("vscode");
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>Available Commands:[ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-Cyril, todoist, trash, settings, sendmsg]";
                }
                break;
            case "echo":
            ...
            case "about-Cyril":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("about-Cyril");
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>Available Commands: [ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-Cyril, todoist, trash, settings, sendmsg ]";
                }
                break;
    ...
    render() {
        return (
            <div className="w-full h-full text-sm font-bold text-white bg-ub-drk-abrgn" id="terminal-body">
                {
                    this.state.terminal
                }
            </div>
